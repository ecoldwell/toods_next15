  ...
  S.list()
    ...
    .items([
      ...
      S.documentTypeListItem('platform')
        .id('platformContent')
        .title('Platforms'),
      S.documentTypeListItem('synchronization')
        .id('syncContent')
      ...
      ...S.documentTypeListItems().filter(
        (item) => item.getId() && !['post', 'category', 'author', 'event', 'artist', 'venue', "page", "faq", "site", 'platform', 'synchronization'].includes(item.getId()!),
      ),
    ])